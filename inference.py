"""
Movie rating model inference with schema validation and logging.
"""
import json
import logging
...
def validate_input(df: pd.DataFrame) -> bool:
    """Validate input DataFrame has numeric and categorical features."""
    if df.empty:
...
def predict(model, df: pd.DataFrame) -> Dict[str, Any]:
    ...
    return {"predictions": predictions.tolist(), "mse": float((predictions**2).mean())}


def main():
    """CLI entrypoint for inference."""
    import argparse
    parser = argparse.ArgumentParser(description="Movie rating prediction inference")
