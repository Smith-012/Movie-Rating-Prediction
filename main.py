...
def infer_target(df: pd.DataFrame) -> str:
    candidates = ["rating", "Rating", "imdb_rating", "score", "user_rating"]
    for col in candidates:
        if col in df.columns:
            return col
    raise ValueError("No rating target found. Use one of: rating, Rating, imdb_rating, score, user_rating")
...
def main(data_path: Path, out_dir: Path) -> None:
    ...
    print("Movie rating portfolio pipeline complete")
...
def cli() -> None:
    parser = argparse.ArgumentParser(description="Portfolio-grade movie rating regression")
    parser.add_argument("--data", type=Path, default=Path("data/movies.csv"))
